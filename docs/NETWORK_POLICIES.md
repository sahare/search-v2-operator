...
| Direction | Peer | Port | Rationale |
|---|---|---|---|
| Ingress | `policy-group.network.openshift.io/host-network` namespaceSelector + empty `podSelector` (OCP's `allow-from-hostnetwork` pattern) | 9443/TCP | The Kubernetes API server calls the operator's `CollectorConfig` admission webhook. The API server uses `hostNetwork: true`. A plain empty `namespaceSelector` does **not** reliably match hostNetwork traffic (confirmed by QE in ACM-37052 — it still blocked the webhook); only OCP's documented host-network label selector, combined with an empty `podSelector` on the same peer, does. |
